        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @if($journal->description)
                                        <div class="text-xs text-gray-500 truncate max-w-xs">{{ $journal->description }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <a href="{{ $journal->base_url }}" target="_blank" class="text-indigo-600 hover:text-indigo-900 hover:underline">
                                        {{ Str::limit($journal->base_url, 30) }}
                                    </a>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        ...
                                        <form action="{{ route('journals.toggle-status', $journal) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" 
                                                    class="{{ $journal->is_active ? 'text-yellow-600 hover:text-yellow-900' : 'text-green-600 hover:text-green-900' }}" 
                                                    title="{{ $journal->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <form action="{{ route('journals.destroy', $journal) }}" method="POST" class="inline" 
                                              onsubmit="return confirm('Apakah Anda yakin ingin menghapus jurnal \'{{ $journal->name }}\'?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center">
                                    ...
                                    <p class="mt-2 text-sm text-gray-500">Belum ada jurnal terdaftar</p>
                                    <a href="{{ route('journals.create') }}" class="mt-2 inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700">
                ...
                <div class="mt-4 text-sm text-gray-500">
                    Total: <span class="font-medium">{{ $journals->count() }}</span> jurnal
                </div>
            </div>
        </div>
    </div>
